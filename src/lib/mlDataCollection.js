...
export async function captureSkillGeneration(data) {
    try {
        // Calculate metadata
        const answeredCount = Object.values(data.responses).filter((a) => a?.trim().length > 0).length;
        const answerLengths = Object.values(data.responses).map((a) => a.split(/\s+/).length);
...
export async function logPDFDownload(questionnaire_id, filename) {
    try {
        await supabase
            .from('skill_generations')
...
export async function submitImmediateFeedback(user_id, questionnaire_id, feedback) {
    try {
        const { error } = await supabase
...
export async function schedule3moFollowup(user_id, questionnaire_id) {
    try {
...
export async function schedule6moFollowup(user_id, questionnaire_id) {
    try {
        const scheduled_at = new Date();
