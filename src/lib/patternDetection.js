...
const QUESTION_CATEGORY_WEIGHT = {
...
export function detectPatterns(responses, questions) {
    ...
    questions.forEach((q) => {
        ...
        SKILL_CLUSTERS.forEach((cluster) => {
            const matchCount = cluster.keywords.filter((kw) => answerLower.includes(kw)).length;
            if (matchCount > 0) {
                ...
                pattern.keywords = Array.from(new Set([
                    ...pattern.keywords,
...
export function calculateConfidenceScore(responses, skill, questions) {
    const answeredCount = Object.values(responses).filter((a) => a?.trim().length > 0).length;
    const totalQuestions = questions.length;
    const completionRate = (answeredCount / totalQuestions) * 100;
    // Higher completion + longer answers = higher confidence
    const avgAnswerLength = Object.values(responses).reduce((sum, a) => sum + a.length, 0) /
        answeredCount;
...
export function generateSkillDescription(skill, responses) {
    const skillDescriptions = {
        'Strategic Thinking': 'You naturally see the big picture, plan systems, and architect solutions that work at scale.',
        'Communication': 'You translate complex ideas into clear messages that resonate and inspire action.',
        'Building & Creation': 'You turn ideas into reality, shipping products and businesses that didn\'t exist before.',
        'Leadership': 'You bring people together, set direction, and create cultures where others thrive.',
        'Problem Solving': 'You diagnose what\'s broken and engineer solutions that actually stick.',
        'Analysis & Research': 'You uncover hidden patterns in data and dig until you find the root truth.',
        'Creative Direction': 'You have a distinctive vision for what\'s possible and the taste to execute it.',
        'Operations & Systems': 'You design workflows and processes that scale, optimize, and handle complexity.',
    };
